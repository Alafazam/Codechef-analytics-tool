...
"""A tool to collect analyse and display data for codechef profiles."""

from flask import Flask, request, render_template
# from flask import session, jsonify, flash, url_for, redirect
...
def get_data(username):
    """Fetch a users data."""
    if re.match('^[a-z]{1}[a-z0-9_]{3,13}$', username) is None:
        return render_template(
            'index.html',
            message="Illegal Username/Input is not alphanumeric")
    ef = 0
    url = 'https://www.codechef.com/recent/user?page=0&user_handle=' + str(
        username)
    try:
...
def generate__data(username):
    """Generate data by sending requests page by page."""
    start_page = int(request.args.get('start')) or 0
    end_page = int(request.args.get('end')) or 1
    # start_page  = 0
    # end_page    = 10
    if re.match('^[a-z]{1}[a-z0-9_]{3,13}$', username) is None:
    ...
    def generate():
        ...
        yield '{ "content":[ { "hours":null, "mins":null, "date_d":null,' + \
              '"date_m":null, "date_y":null, "problem_code":null,' + \
              ' "qstatus":null, "langz":null } '
        for x in range(start_page, end_page + 1):
            time.sleep(0.1)
            print "sending request for page " + str(x)
            url = 'https://www.codechef.com/recent/user?page='
            url += str(x) + '&user_handle=' + str(username)
            try:
                r = s.get(url).json()
